...
### **Platform Design**
General design principles to reference when creating frontend architecture and experiences.
...
### **Header Section** 
Header section with configurable combinations of left/center/right algined options and actions (e.g. title text, back button, notifications button, X button, settings button, slider filter button, ellipses button, cancel button, done button, save button).
- **Header Badges:** Reactive "Badge" system. The notification and filter icons in the Header Section must listen for external activity triggers (e.g., new_notification, applied_filter) to display visual indicators without requiring a full page refresh.


### **Navigation Menu** 
Navigation menu featuring five core modules: Home, Finances, Calendar, Messages, and Discover.
- **Navigation Badges:** Reactive "Badge" system. The messages icon must listen for external activity triggers (e.g., unread_count, new_listing_alert) to display visual indicators without requiring a full page refresh.


### **Home Page**
Page displaying [[insert something here]]


### **Discover Page (aka Roommate Search)** 
Page displaying a "stack" of user profiles and their related property profiles supporting both Tinder-style gestures (swiping) and button interactions to help users find, review, and decide on potential roommates.
- **Discover State Machine:** Sysyem to progress user states along the roommate matchmaking lifecycle using swiping and button interactions; negative actions to the left and affirmative actions to the right
...
- **Discover Ranking & Recommendation Heuristics:** System to rank and sort the order of user profiles displayed on the "Discover" module.
...
- **Discover Search Filters:** 
    - ____
- **User Profile:** Card component containing user details 
- Allow users to effortlessly browse potential user details, and their related roommates' user details and property details. Allow for variations for viewing someone else vs yourself, property vs no proprty, roommates vs no roommates, and varying Discover matchmaking states
    - up to 6 photos
    - user-name
    - user-age
    ...
    - lease-end-date
    - user-id-roommates
- **Property Profile:** Card component containing property details 
- **Edit User:** Page to edit user details
- **Edit Property:** Page to edit property details
